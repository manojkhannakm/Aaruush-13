...
import com.webarch.aaruush13.tiles.game.scenes.main.MainScene;
...
public class GameScene extends BaseScene implements IAccelerationListener, IOnSceneTouchListener, ContactListener {
    ...
    public static short wallCategoryBit = 1, playerCategoryBit = 2, otherCategoryBit = 4, wallMaskBit = (short) (wallCategoryBit + playerCategoryBit), playerMaskBit = (short) (wallCategoryBit + playerCategoryBit + otherCategoryBit), otherMaskBit = (short) (playerCategoryBit + otherCategoryBit);
    ...
    protected void onManagedUpdate(float pSecondsElapsed) {
        ...
        if (isShaking) {
            if (System.currentTimeMillis() < shakeTime) {
                camera.setCenter(cameraHalfWidth + MathUtils.random(15.0f, 25.0f) * shakeIntensity * (MathUtils.random(0.0f, 1.0f) <= 0.5f ? -1 : 1), cameraHalfHeight + MathUtils.random(15.0f, 25.0f) * shakeIntensity * (MathUtils.random(0.0f, 1.0f) <= 0.5f ? -1 : 1));
            } else {
                isShaking = false;
                shakeTime = 0;
                shakeIntensity = 0.0f;
